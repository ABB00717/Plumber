...
class Maze {
    ...
    void setting() {
        ...
        std::cout << "Please input the row size (must be greater than 3 and smaller than 20): ";
        std::getline(std::cin, rowStr);
        while (!isNumber(rowStr) || std::stoi(rowStr) < 3 || std::stoi(rowStr) > 20) {
            std::cout << "Does not meet input requirements, please try again: ";
            std::getline(std::cin, rowStr);
        }

        std::cout << "Please input the col size (must be greater than 3 and smaller than 20): ";
        std::getline(std::cin, colStr);
        while (!isNumber(colStr) || std::stoi(colStr) < 3 || std::stoi(colStr) > 20) {
            std::cout << "Does not meet input requirements, please try again: ";
            std::getline(std::cin, colStr);
        }
